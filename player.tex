...
avait été fixée à la hiérarchie présentée figure~\ref{videostitch-apps}.\\
Le dossier de compilation est fixé à bin/x64/<configuration> pour tous les systèmes~: Windows, Linux 
et Mac OS X.
\begin{figure}
  \dirtree{%
    .1 /.
...
En effet, les applications définissent des projets Qt (extension .pro) décrivant
...
des projets Qt~: un seul projet est écrit, tout en couvrant plusieurs systèmes.
\ \\
\newline
...
utilisé pour gérer son code et ses modifications, et utilisé en ligne de commande,
...
les scripts bash. Si le support de bash est supporté sur tous les Linux et les Mac OS X,
l'utilisation systèmatique de Cygwin sur Windows par l'équipe a assuré ce support
